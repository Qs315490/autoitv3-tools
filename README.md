...
> **拖动时指针拥有几何**：用户正在拖的那个窗口不套用模型的位置——脚本只能把上一帧上报的
> 位置写回模型，套用它就会把窗口拽回"一拍之前"的地方，表现就是**窗口跟不上鼠标、拖动发抖**。
> 判定是"指针在上一帧画出的那个矩形里"（`DrawnWindow::pointer_owns`，归属记到松开为止），
> 所以脚本在用户拖 A 的同时移动 B 照样有效。这些帧也照常上报，模型的延迟不会越积越多。
