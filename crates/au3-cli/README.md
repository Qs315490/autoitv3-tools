...
| `unpack <PATH> [--dir DIR] [--by-type] [--all] [--payload] [-o FILE]` | `unp` | 取回产物里带的东西：**默认先解出编译脚本，再按脚本里 `#AutoIt3Wrapper_Res_File_Add` 指令落盘**——第一参数是 wrapper 构建时的 staging 路径（`__ResImage\\_NAME`/`__Res64\\NAME`/`__NAME`），第三参数是资源名，字节写回第一参数的路径，解出来的目录就是一份 staging 目录（`--payload` 直接认）；脚本没点名或点名缺资源会报错。脚本解不出/没有指令时退回全量（全部资源按 staging 布局），`--all` 强制全量、`--by-type` 按类型分目录；默认写输入旁的 `<input>.unpacked/`，`--dir DIR` 换位置。`--script` 输出编译进去的 `.au3` 源码；`--payload` 解码那个 4 资源的打包载荷（`--raw`/`--table`/`--at`/`-o` 作用于它）。UPX 加壳的镜像会被点名（`UPX0`/`UPX1`/`UPX!`），它不会替你脱壳 |
