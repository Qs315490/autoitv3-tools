...
| `stopat <函数>` / `sa` | **在调用之前**停下：内置函数停在它执行前（`stopat MsgBox` 把要弹的对话框内容连同它所在的行先打出来，对话框不会弹），脚本函数停在它的入口（参数已经绑定，`print $x` 直接能读）。`continue` 之后照常执行；`stopat` 单独用是查看，`stopat off` 是取消 |
