...
/// The processor architecture the emulation claims.
///
/// This drives `@OSArch`/`@ProcessorArch`, `@CPUArch`, the pointer-sized
/// fields of `DllStruct*`, and the `wProcessorArchitecture` that
/// `GetSystemInfo` reports.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum WindowsArch {
    /// 32-bit x86.
    X86,
    /// 64-bit x64 — the default, matching the reference sample's
    /// `Win10.x64` / `Win11.x64`.
    #[default]
    X64,
    /// 64-bit ARM.
...
impl WindowsArch {
    ...
    pub fn from_name(name: &str) -> Option<Self> {
        match normalize(name).as_str() {
            "x86" | "i386" | "i486" | "i586" | "i686" | "win32" | "32" => Some(WindowsArch::X86),
            "x64" | "amd64" | "x8664" | "64" => Some(WindowsArch::X64),
            "arm64" | "aarch64" => Some(WindowsArch::Arm64),
            _ => None,
        }
    }
}
