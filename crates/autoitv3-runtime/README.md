...
1. **小型解释器**（`interp.rs`）——供反混淆调用。覆盖 `ByRef`（拷入/拷出，含数组元素，
   ...
   数组声明按官方实测：显式维度就是形状，空括号（`[]`）按初始值在该层的长度定维——
   `Local $a[][2] = [[1,2],[3,4],[5,6]]` 是 3x2、`Local $a[2][] = [[1,2],[3,4]]` 是 2x2、
   `Local $a[][][] = [[[1,2],[3,4]],[[5,6],[7,8]]]` 是 2x2x2；比声明短的初始值补空串，
   比声明长的报错，`Local $a[3] = 5` 这种标量初始值也是错误。
   指针也在这里：AutoIt 的 `Ptr` 是**基类型**，`Value::Ptr` 就是它——`ptr` 返回、
   `ptr*` 出参、`Ptr` / `DllStructGetPtr` / `DllCallbackGetPtr` 的结果带上这个类型，
   所以 `IsPtr` 按类型回答、转字符串给十六进制（`0x` + 16 位大写，官方实测），而算术、
   比较、当 Map 键仍与整数一致。`HWnd` 基类型未建模（`IsHwnd` 恒 0）。

   比较语义同样按官方实测：`=`/`<>` 先做一次大小写不敏感的**字符串**比较，不等再退回**数值**
   比较（字符串按前导数字解析，没有前导数字的记 0）——所以 `"" = 0`、`"abc" = 0` 为真，
   而两边都是字符串的 `"" = "0"` 为假；字符串比较在前，这就是 `"true" = True` 为真的原因。
   `Null`/`Default` 只等于自身；`==` 是纯的大小写敏感（两边都是数字时按数值）比较；
   `<` 一类混合类型一律按数值比较（`"10" < "9"` 真、`"10" < 9` 假）。
...
### 启动钩子：`#OnAutoItStartRegister`

程序顶层带 `#OnAutoItStartRegister "函数"` 时，`Runtime::load_program` 记下这个名字，
`run_script` 在跑本体第一条语句**之前**调用它一次（每次启动都算一次，所以调试器里
`run` 重启也会再调）。名字大小写不敏感、可带引号；函数不存在就按普通的未定义函数报错。
指令只看顶层，和 AutoIt 的前处理器一致（`#region` 里算，函数体里不算）。
